...
def update_summary_from_episodes(client, current_summary, episodes, rewards, model="unsloth/gemma-4-26B-A4B-it-GGUF", seed=None):
    ...
    try:
        ...
        ai_agent.log_interaction(log_label, messages, response_text, seed)
        ai_agent.save_summary(response_text, seed)
        return response_text
    except Exception as e:
        ai_agent.log_interaction(log_label + " ERROR", messages, str(e), seed)
...
def train(n_steps=2048, n_epochs=4, batch_size=64,
    ...
    # Seed from whatever summary_{seed}.md already has (e.g. from ai_agent.py
    # warm start with the same seed). None is fine — update_summary_from_episodes
    # will bootstrap one.
    summary_text = ai_agent.load_summary(seed)
    ...
    while ep_count < max_episodes:
        ...
        if mean_100 > best_mean:
            ...
            torch.save({k: v.cpu() for k, v in actor.state_dict().items()},
                       os.path.join(save_dir, f"actor_best_llm_{seed}.pt"))
        ...
        if ep_count - last_plot_ep >= plot_every and all_ep_rewards:
            ...
            plot_rewards(all_ep_rewards,
                         save_path=os.path.join(save_dir, "logs", f"rewards_llm_{seed}.png"))
...
if __name__ == "__main__":
    ...
    crash_log = open(os.path.join(save_dir, f"crash_trace_llm_{args.seed}.log"), "w")
    faulthandler.enable(file=crash_log, all_threads=True)

    log_path = os.path.join(save_dir, f"terminal_output_llm_{args.seed}.md")
